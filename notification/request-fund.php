...
class RequestFund extends \SejoliSA\Notification\Main {
    ...
    protected function set_content() {
		...
		$this->set_recipient_title  ('buyer', 'email', sejolisa_carbon_get_theme_option('request_fund_email_title'));
		$this->set_recipient_content('buyer', 'email', $this->set_notification_content(
												sejolisa_carbon_get_theme_option('request_fund_email_content'),
												'email',
                                                'buyer'
											 ));

		if(!empty(sejolisa_carbon_get_theme_option('request_fund_whatsapp_content'))) :

            $this->set_enable_send('whatsapp', 'buyer', true);
			$this->set_recipient_content('buyer', 'whatsapp', $this->set_notification_content(
		                                                sejolisa_carbon_get_theme_option('request_fund_whatsapp_content'),
		                                                'whatsapp',
                                                        'buyer'
                                                    ));
        endif;

		if(!empty(sejolisa_carbon_get_theme_option('request_fund_sms_content'))) :
            $this->set_enable_send('sms', 'buyer', true);
			$this->set_recipient_content('buyer', 'sms', $this->set_notification_content(
                                    				sejolisa_carbon_get_theme_option('request_fund_sms_content'),
                                    				'sms',
                                                    'buyer'
                                    			));
        endif;
		...
		if(false !== sejolisa_carbon_get_theme_option('request_fund_admin_active')) :

            $this->set_enable_send('email', 'admin', true);
    		$this->set_recipient_title('admin', 'email', sejolisa_carbon_get_theme_option('request_fund_admin_email_title'));
    		$this->set_recipient_content('admin', 'email', $this->set_notification_content(
													sejolisa_carbon_get_theme_option('request_fund_admin_email_content'),
													'email',
                                                    'admin'
												));

    		if(!empty(sejolisa_carbon_get_theme_option('request_fund_admin_whatsapp_content'))) :

				$this->set_enable_send('whatsapp', 'admin', true);
				$this->set_recipient_content('admin', 'whatsapp', $this->set_notification_content(
                                                			sejolisa_carbon_get_theme_option('request_fund_admin_whatsapp_content'),
                                                			'whatsapp',
                                                            'admin'
                                                		));
            endif;

    		if(!empty(sejolisa_carbon_get_theme_option('request_fund_admin_sms_content'))) :
                $this->set_enable_send('sms', 'admin', true);
				$this->set_recipient_content('admin', 'sms', $this->set_notification_content(
                                        				sejolisa_carbon_get_theme_option('request_fund_admin_sms_content'),
                                        				'sms',
                                                        'admin'
                                        			));
            endif;
    ...
    protected function check_recipients($media = 'email', $role = 'admin') {
        ...
        $recipients       = sejolisa_carbon_get_theme_option('request_fund_' . $role . '_' . $media . '_recipient');
